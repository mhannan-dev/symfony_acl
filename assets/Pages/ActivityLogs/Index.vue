        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="log in logs" :key="log.id" class="hover:bg-slate-50">
              <td class="px-6 py-4 text-sm text-slate-500 whitespace-nowrap">{{ new Date(log.actionTime.date).toLocaleString() }}</td>
              <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ log.user?.name || '—' }}</td>
              ...
              <td class="px-6 py-4 text-sm text-slate-500">{{ log.contentType?.appLabel || '—' }}</td>
            </tr>
            <tr v-if="!logs.length">
