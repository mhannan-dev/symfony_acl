  ...
  <Layout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <Link href="/admin/users/new" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg px-4 py-2 transition-colors">
          Add User
        </Link>
      </div>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="user in users" :key="user.id" class="hover:bg-slate-50">
              ...
              <td class="px-6 py-4 text-sm text-slate-500">
                ...
                <span v-if="!user.userGroups.length" class="text-slate-400">—</span>
              </td>
              <td class="px-6 py-4 text-right">
                <Link :href="`/admin/users/${user.id}/edit`" class="text-sm text-blue-600 hover:text-blue-800 font-medium mr-3">Edit</Link>
