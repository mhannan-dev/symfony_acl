  ...
  <Layout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <Link href="/admin/content-types/new" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg px-4 py-2 transition-colors">
          Add Content Type
        </Link>
      </div>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="ct in contentTypes" :key="ct.id" class="hover:bg-slate-50">
              ...
              <td class="px-6 py-4 text-right">
                <Link :href="`/admin/content-types/${ct.id}/edit`" class="text-sm text-blue-600 hover:text-blue-800 font-medium mr-3">Edit</Link>
...
function deleteContentType(id) {
  if (confirm('Are you sure you want to delete this content type?')) {
    router.post(`/admin/content-types/${id}/delete`)
  }
}
</script>
