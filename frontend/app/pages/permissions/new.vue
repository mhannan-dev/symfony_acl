<template>
  <div class="max-w-2xl space-y-6">
    <div class="flex items-center gap-4">
      ...
      <h1 class="text-2xl font-bold text-slate-900">New Permission</h1>
    </div>
...
onMounted(async () => {
  const { data } = await get<{ contentTypes: any[] }>('/permissions/new')
...
async function submit() {
  ...
  const { error } = await post('/permissions/save', {
    ...
    contentTypeId: form.contentTypeId ? parseInt(form.contentTypeId) : null,
  })
  saving.value = false
  if (error) {
  ...
  useToast().success('Permission created successfully')
