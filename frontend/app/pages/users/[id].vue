<template>
  <div class="max-w-2xl space-y-6">
    ...
    <form @submit.prevent="submit" class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
      ...
      <div>
        <label class="block text-sm font-medium text-slate-700 mb-1.5">New Password (leave blank to keep)</label>
...
onMounted(async () => {
  const { data } = await get<{ user: any; groups: any[]; userGroupIds: number[] }>(`/users/${route.params.id}/edit`)
  if (data) {
    form.id = data.user.id
    form.name = data.user.name
    form.email = data.user.email
    form.groupIds = data.userGroupIds || []
