  ...
  <Layout>
    <div class="max-w-2xl space-y-6">
      <div class="flex items-center gap-4">
        <Link href="/admin/content-types" class="text-slate-400 hover:text-slate-600 transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
          </svg>
        </Link>
        <h1 class="text-2xl font-bold text-slate-900">{{ contentType ? 'Edit Content Type' : 'New Content Type' }}</h1>
      </div>
      ...
      <form @submit.prevent="submit" class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
        ...
        <div class="flex items-center gap-3 pt-2">
          ...
          <Link href="/admin/content-types" class="text-sm text-slate-500 hover:text-slate-700 font-medium">Cancel</Link>
        </div>
      </form>
    </div>
  </Layout>
...
const form = reactive({
  id: props.contentType?.id || null,
  appLabel: props.contentType?.appLabel || '',
  model: props.contentType?.model || '',
  processing: false,
})

function submit() {
  form.processing = true
  router.post('/admin/content-types/save', {
    id: form.id,
