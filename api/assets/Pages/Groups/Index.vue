  ...
  <Layout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <Link href="/admin/groups/new" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg px-4 py-2 transition-colors">
          Add Group
        </Link>
      </div>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="group in groups" :key="group.id" class="hover:bg-slate-50">
              ...
              <td class="px-6 py-4 text-right">
                <Link :href="`/admin/groups/${group.id}/edit`" class="text-sm text-blue-600 hover:text-blue-800 font-medium mr-3">Edit</Link>
...
function deleteGroup(id) {
  if (confirm('Are you sure you want to delete this group?')) {
    router.post(`/admin/groups/${id}/delete`)
  }
}
</script>
