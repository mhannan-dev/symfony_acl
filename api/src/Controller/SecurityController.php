...
class SecurityController extends AbstractController
{
    ...
    public function login(AuthenticationUtils $authenticationUtils, InertiaInterface $inertia, Request $request): Response
    {
        ...
        if ($request->hasSession()) {
            ...
            if ($session->has('_login_errors')) {
                $errors = (object)$session->get('_login_errors');
                $session->remove('_login_errors');
            }

            $authError = $authenticationUtils->getLastAuthenticationError();
            if ($authError && empty((array)$errors)) {
                $errors = (object)['email' => $authError->getMessageKey()];
            }
        }
    ...
    #[Route(path: '/auth/login', name: 'app_login_post', methods: ['POST'])]
    public function loginPost(
        ...
        dd($request->request->all(), $request->getContent());
        if ($this->getUser()) {
            return $this->redirectToRoute('app_home');
        }
        ...
        if (!$user || !$passwordHasher->isPasswordValid($user, $loginRequest->password)) {
            $request->getSession()->set('_login_errors', [
